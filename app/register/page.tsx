...
const fields = [
    ...
    { type: 'tel', placeholder: 'Enter your phone number', label: 'Phone Number' },
    { type: 'password', placeholder: 'Enter your password', label: 'Password' },
    { type: 'text', placeholder: 'Enter referral code (Optional)', label: 'Referral Code' },
];
export default function RegisterPage() {
    return (
        <section className="w-full lg:h-dvh lg:p-0 md:p-10 p-5 relative flex justify-center items-center lg:text-gray-900 text-white">
            <div className=" bg-white/30 backdrop-blur-xl w-full rounded-lg p-10 lg:p-0 lg:px-10 lg:py-6 lg:overflow-y-auto flex-1/2 space-y-2 lg:space-y-1 lg:h-full lg:rounded-none lg:bg-white">
                <form className="space-y-3">
                    <h1 className="text-3xl capitalize">Create account</h1>
                    {
                        fields.map((field) => (
                            <InputField key={field.placeholder} type={field.type} placeholder={field.placeholder} label={field.label} />
                        ))
                    }
                    <div className="flex items-center gap-2">
                        <input type="checkbox" className="w-5 h-5" />
                        <small>I agree to Ogaticket <span className="text-primary"><a href="">terms & policy</a></span></small>
                    </div>
                    <div>
                        <button className="w-full cursor-pointer text-white bg-primary py-3 rounded-lg">Create an Account</button>
                    </div>
                </form>
                <div>
                    <p>
                        Already have an account? <span className="text-primary"><Link href="/login">login</Link></span>
                    </p>
                </div>
...
const InputField = ({ type, placeholder, label }: InputProps) => {
    return (
        <div>
            <label className="block text-sm text-white lg:text-gray-600" htmlFor={label}>{label}</label>
            <input
